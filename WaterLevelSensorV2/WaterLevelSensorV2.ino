...
/*
  ...
  Calibration information  (to be rechecked)
  -- Transfer function V_OUT = V_S * ( 0.09 P + 0.04 ) or P = ((V_OUT/V_S) - 0.04))/0.09 kPa.   i.e. 4.7 V = 10.88 kPa, 0.2V = 0.88 kPa
  -- In SI units : P (Pa) = ((V_OUT/V_S) - 0.04) /0.00009   :
  ...
  -- Tank head in m = Pressure / 9810 = ((V_out/V_s) -0.04)/0.00009)/9810 = (V_out) / (V_s * 0.00009 * 9810) - 0.04/(0.00009*9810)
  -- Height (m) = (1.13 V_out ) - 0.045
  -- Height (cm) = (113 V_out ) - 4.5
...
void lcd_interface()
{
  sprintf (LcdTopRowBuffer, "%s: A0: %4i", msgPrefix[nWarnLevel], nCurrentLevel);
  sprintf (LcdBottomRowBuffer, "%s: A0: %4i", msgPrefix[nWarnLevel], nCurrentLevel);
  lcd.setCursor(0, 0);
  lcd.print(LcdTopRowBuffer);
  lcd.setCursor(0, 1);
  lcd.print(LcdBottomRowBuffer);
  Serial.println("wrote to lcd");
  Serial.println(LcdTopRowBuffer);
  Serial.println(LcdBottomRowBuffer);
